...
The dominant prior art in media provenance is C2PA (Coalition for Content Provenance and Authenticity), the Content Credentials standard backed by Adobe, Microsoft, the BBC, and others. Gemma.Witness shares the cryptographic building blocks (SHA-256 over raw bytes, public-key signatures over a canonicalized manifest, schema versioning) and diverges on three properties documented in [`docs/comparison-with-c2pa.md`](../docs/comparison-with-c2pa.md): the sealed reasoning trace, the multi-asset incident bundle, and the zero-network verifier. The README has a shorter framing under [§"How this compares to C2PA"](../README.md#how-this-compares-to-c2pa). Implementers who plan to interoperate with C2PA-aware tooling should start there.
