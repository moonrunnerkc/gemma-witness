...
async function runTests(): Promise<void> {
  ...
  console.log("--- T1 (positive): valid fixture bundle");
  {
    ...
    console.log("T1 outcome:", JSON.stringify(outcome, null, 2));
  ...
  // T4: Unknown model fingerprint (empty known list).
  console.log("--- T4 (negative): unknown model fingerprint");
  {
  ...
  console.log("--- T5 (canonical): reorder manifest keys");
  {
    const mutated = mutateBundle((entries) => {
      ...
      entries.set(
        "manifest.json",
        new TextEncoder().encode(JSON.stringify(reordered, null, 2)),
      );
    });
    const outcome = await verifyBundle(mutated, KNOWN_WITH_FINGERPRINT);
