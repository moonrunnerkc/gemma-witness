...
This is a pre-release research project. Read the [Known issues and limitations](#known-issues-and-limitations) section before relying on it.
...
CI runs the Rust suites on `macos-latest`; see `.github/workflows/ci.yml`. The verifier e2e suite runs on `ubuntu-latest` and exercises 7 cases including signature corruption, asset byte flips, manifest key reordering, unknown fingerprint, and unknown `manifest_version`.
...
The following are present in the current codebase. They do not block development but they affect what you can trust a bundle to prove.
...
11. **Sidecar model identifiers are not bundled.** The code, scripts, and documentation refer to `google/gemma-4-E4B-it` and `mlx-community/gemma-4-e4b-it-4bit`. Weights are fetched from Hugging Face on first sidecar startup; if those identifiers are unavailable or renamed in your environment, the sidecar will fail to start. Override with `GW_SIDECAR_MODEL`.

## What you can verify yourself

The repository is structured so that the important claims are checkable from a fresh clone without running the capture app.

- `cd apps/verifier && pnpm install && pnpm build` rebuilds the static verifier. `build.mjs` fails the build if the output HTML contains any external `src`/`href`, any `fetch`, any `XMLHttpRequest`, or any `importScripts`.
- `cd apps/verifier && npx tsx tests/e2e.test.ts` runs the verifier against the committed fixture `tests/fixtures/day-4-fixture.witness` and against deliberately tampered variants produced in-test.
- `cargo test --workspace` runs the Rust unit and integration suites: canonicalization round-trips, bundle round-trips, keystore behavior, manifest schema validation.
- `cargo test -p witness-core --test day-4-e2e` runs the live pipeline if a sidecar is reachable on `127.0.0.1:8080`, or prints a skip notice if not.
- `cat spec/bundle-format.md spec/manifest-schema.json spec/incident-schema.json` is the full on-the-wire contract.

Throughput, memory footprint, and latency are workload- and hardware-dependent and are not benchmarked in this repository.
